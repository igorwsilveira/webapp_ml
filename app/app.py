...
def make_picture(training_data_filename, model, new_inp_np_arr, output_file):
  ...
  fig = px.scatter(x=ages, y=heights, title="Height vs Age of People", labels={'x': 'Age(Years)', 'y': 'Height (Centimeters)'})

  fig.add_trace(go.Scatter(x=x_new.reshape(19), y=preds, mode='lines', name='Model'))

  new_preds = model.predict(new_inp_np_arr)

  fig.add_trace(go.Scatter(x=new_inp_np_arr.reshape(len(new_inp_np_arr)), y=new_preds, name='New Outputs', mode='markers', marker=dict(color='red', size=20)))
